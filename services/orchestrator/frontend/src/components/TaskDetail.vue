  ...
  <el-card v-if="task" shadow="never" class="task-summary">
    <template #header>
      <header class="summary-header">
        ...
        <div class="summary-tags"><el-tag :type="statusType(task.status)" effect="dark">{{ task.status }}</el-tag><el-tag :type="task.executionMode === 'READ_ONLY' ? 'warning' : 'danger'">{{ task.executionMode }}</el-tag></div>
      </header>
    </template>
...
@media (max-width: 760px) { .summary-grid, .compact-facts { grid-template-columns: 1fr; }.summary-card--wide { grid-column: auto; }.summary-header, .card-heading { flex-direction: column; }.summary-tags { justify-content: flex-start; }.recent-events li { grid-template-columns: auto minmax(0, 1fr); }.recent-events time { grid-column: 2; } }
</style>
