...
export function statusTone(status: string | null | undefined): StatusTone {
  ...
  if (['SUCCESS', 'COMPLETED', 'ACTIVE', 'APPROVED', 'CONSUMED', 'DONE'].includes(value)) return 'success'
  if (['FAILED', 'ERROR', 'REJECTED'].includes(value)) return 'danger'
  if (['RUNNING', 'CODING', 'TESTING', 'CONVERTED'].includes(value)) return 'info'
  ...
  if (['RECOVERY_REQUIRED', 'RETRY_WAIT', 'STARTING', 'BLOCKED'].includes(value)) return 'warning'
  if (['PENDING', 'PLANNING', 'PLANNED', 'IDEA', 'CREATED', 'QUEUED', 'WAITING_APPROVAL'].includes(value)) return 'warning'
