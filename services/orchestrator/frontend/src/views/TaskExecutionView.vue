...
import { getDiagnosis, type FailureDiagnosis } from '../api/diagnosis'
...
const diagnosisDetailsVisible = ref(false)
...
async function loadExecutionState(): Promise<void> {
  ...
  // V1 Failure Diagnosis：无失败时返回 undefined → null
  diagnosis.value = (await getDiagnosis(taskId).catch(() => undefined)) ?? null
}
...
<template><section class="page-stack">
  ...
  <template v-if="context.task.value">
    ...
    <section class="workflow-summary" aria-label="Workflow Summary">
      ...
      <section v-if="diagnosis" class="failure-diagnosis" aria-label="Failure Diagnosis">
        <div class="review-header"><div><p class="page-eyebrow">Failure</p><h2>{{ diagnosis.code }}</h2></div><div style="display:flex;gap:.5rem;align-items:center"><StatusBadge :status="diagnosis.category" /><span v-if="diagnosis.knownFailure" class="known-failure-tag">KNOWN FAILURE · Seen {{ diagnosis.occurrenceCount }} times</span><span v-else class="new-failure-tag">NEW FAILURE</span></div></div>
        <dl class="review-grid">
          <div><dt>Stage</dt><dd>{{ diagnosis.stage }}</dd></div>
          <div><dt>Error</dt><dd><code>{{ diagnosis.errorCode }}</code></dd></div>
        </dl>
        <p class="diagnosis-root"><strong>Root Cause:</strong> {{ diagnosis.rootCause }}</p>
        <ul v-if="diagnosis.evidence.length" class="changed-files"><li v-for="item in diagnosis.evidence" :key="item">{{ item }}</li></ul>
        <div class="review-actions" style="margin-top:.5rem">
          <el-button v-if="diagnosis.recommendedAction === 'RETRY'" type="primary" :loading="interventionBusy" @click="runIntervention('RETRY')">Retry</el-button>
          <el-button v-if="diagnosis.recommendedAction === 'REPLAN'" type="primary" :loading="interventionBusy" @click="runIntervention('REPLAN')">Replan</el-button>
          <el-button @click="diagnosisDetailsVisible = true">Details</el-button>
        </div>
      </section>
