...
<template><section class="page-stack">
  ...
  <template v-if="context.task.value">
    ...
    <el-card v-for="(record, index) in executions.records.value" :key="record.id" shadow="never" class="record-card">
      <template #header><div class="record-header"><div><p class="page-eyebrow">Attempt {{ index + 1 }}</p><h2>{{ record.status }}</h2></div><StatusBadge :status="record.status" /></div></template>
      <dl class="record-grid"><div><dt>Agent</dt><dd>{{ record.agentName || 'Unknown' }}</dd></div><div><dt>Actual Executor</dt><dd>{{ record.executorName || 'Unknown' }}</dd></div><div><dt>Workspace</dt><dd>{{ record.workspace || 'Unknown' }}</dd></div><div><dt>Exit Code</dt><dd>{{ record.exitCode ?? '—' }}</dd></div></dl>
