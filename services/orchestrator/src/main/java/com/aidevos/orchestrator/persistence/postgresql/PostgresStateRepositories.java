...
class PostgresPlanRunRepository implements PlanRunRepository {
	...
	public String findRunIdByApproval(String id){return store.allBySecondary(TYPE,id,PersistenceSnapshots.Run.class).stream().map(PersistenceSnapshots.Run::value).map(PlanRun::getId).findFirst().orElse(null);}
