...
public class RemotePushApprovalService {
    ...
    public synchronized RemotePushApproval approve(String id){RemotePushApproval v=require(id);String from=v.getStatus().name();v.approve();repository.save(v);if(!from.equals(v.getStatus().name()))flow(EventType.REMOTE_PUSH_APPROVAL_APPROVED,v,from,v.getStatus().name());return v;}
